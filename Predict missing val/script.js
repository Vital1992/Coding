...
// There are exactly twenty rows marked missing in each input file. The missing values are marked as "Missing_1",
// "Missing_2", ..., "Missing_20". These missing records have been randomly dispersed in the rows of data.
...
const data = [
    ...
    function findPrev(idx, arr) {
        let prevNum = -1
        for (let i = idx - 1; i >= 0; i--) {
            if (!arr[i].includes('Missing')){
                prevNum = arr[i]
                break
            }
        }
    ...
    function extractTemps(arr) {
        const temps = []
        for (let i = 0; i < arr.length; i++){
            temps.push(arr[i].split(' ')[1].slice(9, arr[i].split(' ')[1].length))
        }
        return temps
    }
    
    function calcMissing(readings) {
        const arr = extractTemps(readings)
        for (let i = 0; i < arr.length; i++){
            if (arr[i].includes('Missing')){
                let prevTemp = findPrev(i, arr)
                let nextTemp = findNext(i, arr)
                if (prevTemp === -1 || nextTemp === -1) {
                    continue
                }
                // console.log(prevTemp)
                // console.log(nextTemp)
                console.log((parseFloat(prevTemp) + parseFloat(nextTemp)) / 2)
            }
    ...
    //       const [date, reading] = entry.split('\t');
    //       return { date, reading };
    //     });
      
    //     for (let i = 0; i < values.length; i++) {
    //       if (values[i].reading.includes('Missing')) {
    //         const missingIndex = parseInt(values[i].reading.split('_')[1]) - 1;
      
    //         let prevIndex = i - 1;
    //         let nextIndex = i + 1;
    //         let prevValue = parseFloat(values[prevIndex].reading);
    //         let nextValue = parseFloat(values[nextIndex].reading);
      
    //         while (isNaN(prevValue)) {
    //           prevIndex--;
    //           prevValue = parseFloat(values[prevIndex].reading);
    //         }
      
    //         while (isNaN(nextValue)) {
    //           nextIndex++;
    //           nextValue = parseFloat(values[nextIndex].reading);
    //         }
      
    //         const missingReading = (prevValue + nextValue) / 2;
    //         missingValues[missingIndex] = missingReading.toFixed(2);
    //       }
    //     }
      
    //     // Print the missing values
    //     missingValues.forEach((value, index) => {
    //       console.log(`Missing_${index + 1}: ${value}`);
    //     });
    //   }
